...
- **Benchmark Guardrail:**
  - `npm run bench:address` — Seeds a deterministic regtest dataset, runs the LevelDB indexer benchmark harness, and writes results to `bench/current-results.json`.
    ...
    - When validating a PR locally, compare the new throughput figures against the previous baseline; call out deltas greater than 10% in the PR description and include relevant tuning parameters from the `settings` block.
    - Tune address sync experiments with `ADDRESS_INDEXER_CONCURRENCY` (prevout workers), `ADDRESS_INDEXER_PARALLEL_ENABLED` (toggle worker-thread prevout fetches), `ADDRESS_PREVOUT_CACHE_MAX` / `ADDRESS_PREVOUT_CACHE_TTL` (short-lived prevout cache sizing), `ADDRESS_LEVEL_CACHE_MB` / `ADDRESS_LEVEL_WRITE_BUFFER_MB` (LevelDB cache + write buffer), `ADDRESS_INDEXER_BATCH_BLOCKS` (number of blocks per batch write), and `BITCOIN_RPC_MAX_SOCKETS` when pairing higher RPC concurrency with increased timeouts. Record any non-default values alongside the benchmark results so QA can reproduce.
    - Inspect `metrics.recordAddressIndexerPrevoutDuration` output (available via `/metrics` when enabled) to ensure duration histograms reflect the expected source (`rpc`, `cache`, or `mixed`) and latency distribution after changes to the worker pool or batching behaviour.
